...
enum class VT100Sequence(val syntax: String, // Just for documentation
                         ...
                         val noBracket: Boolean = false, // Codes doesnt have a opening bracket and fixed content after it
    ...
    RESET_DEVICE(               "<ESC>c", 'c', noBracket = true),
    ENABLE_LINE_WRAP(           "<ESC>[7h", 'h', codePrefixValue = 7),
    DISABLE_LINE_WRAP(          "<ESC>[7l", 'l', codePrefixValue = 7),
    FONT_SET_G0_DEFAULT(        "<ESC>(", '(', noBracket = true),
    FONT_SET_G1_ALTERNATE(      "<ESC>)", ')', noBracket = true),
    CURSOR_HOME(                "<ESC>[H", 'H'),
    ...
    SAVE_CURSOR_AND_ATTRS(      "<ESC>7", '7', noBracket = true),
    RESTORE_CURSOR_AND_ATTRS(   "<ESC>8", '8', noBracket = true),
    ...
    SET_TAB_(                   "<ESC>H", 'H'),
    CLEAR_TAB_(                 "<ESC>[g", 'g'),
    CLEAR_ALL_TABS_(            "<ESC>[3g", 'g', codePrefixValue = 3),
    ...
    val rawPrefix by lazy { "$ESC${if(noBracket) "" else "["}" }
    ...
    fun parseAttributes(sequence: String): Array<Int>? {
        ...
        val rawAttributes = sequence.substring(rawPrefix.length, sequence.length - rawPrefix.length - rawSuffix.length)
